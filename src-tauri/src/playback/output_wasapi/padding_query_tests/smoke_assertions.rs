...
pub fn assert_success_prereq_steps(report: &WasapiPaddingQuerySmokeReport) {
    ...
    assert!(
        ...
        "mix format should be available"
    );
    assert!(
        report.initialize_attempted,
        "initialize should have been attempted"
    );
    assert!(
        report.initialized_audio_client,
        "initialize should have succeeded"
    );
    assert!(
        report.get_service_attempted,
        "get_service should have been attempted"
    );
    assert!(
        report.render_client_obtained,
        "render_client should have been obtained"
    );
    assert!(
        report.get_buffer_size_attempted,
        "get_buffer_size should have been attempted"
    );
    assert!(
        report.buffer_size_frames.is_some(),
        "buffer_size_frames should be Some"
    );
    assert!(
        report.buffer_size_frames.unwrap() >= 1,
        "buffer_size_frames should be >= 1"
    );
}
...
pub fn assert_success_start_stop_and_padding(report: &WasapiPaddingQuerySmokeReport) {
    ...
    assert!(
        report.get_current_padding_attempted,
        "padding query should have been attempted"
    );
    assert!(
        report.padding_frames.is_some(),
        "padding_frames should be Some"
    );
    assert_eq!(
...
pub fn assert_prohibited_always_false(report: &WasapiPaddingQuerySmokeReport) {
    ...
    assert!(
        ...
        "reset_audio_client should be false"
    );
    ...
    assert!(
        ...
        "capability_exposed should be false"
    );
