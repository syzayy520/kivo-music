//! Tests for render loop single-step scaffold.
...
/// A: disabled step skips tick entirely.
#[test]
fn render_loop_disabled_skips_tick() {
    ...
    let report = run_manual_drain_render_loop_step(&mut sink, plan).unwrap();

    assert!(!report.attempted_tick);
    assert_eq!(
        report.skipped_reason,
        WasapiRenderLoopStepSkipReason::Disabled
    );
    assert!(report.tick_report.is_none());
    assert_eq!(report.pending_before, 5);
    assert_eq!(report.pending_after, 5);
    assert_eq!(sink.runtime.pending_frames, 5);
}

/// B: enabled + zero requested → tick called, tick skips with RequestedZero.
#[test]
fn render_loop_enabled_zero_requested_calls_tick_and_tick_skips() {
    let mut sink = WasapiOutputSink::new();
    sink.runtime.pending_frames = 5;
    ...
    assert!(report.attempted_tick);
    assert_eq!(report.skipped_reason, WasapiRenderLoopStepSkipReason::None);

    let tick_report = report.tick_report.as_ref().unwrap();
...
/// C: enabled + no pending → tick called, tick skips with NoPendingFrames.
#[test]
fn render_loop_enabled_no_pending_calls_tick_and_tick_skips() {
    let mut sink = WasapiOutputSink::new();
...
/// D: enabled + pending + no ring buffer → tick attempts drain, returns no-op.
#[test]
fn render_loop_enabled_pending_attempts_tick() {
    let mut sink = WasapiOutputSink::new();
...
/// E: tick error propagates, pending preserved.
#[test]
fn render_loop_tick_error_propagates() {
    let mut sink = WasapiOutputSink::new();
    ...
    let result = run_manual_drain_render_loop_step(&mut sink, plan);
...
/// F: no implicit drain — state untouched without explicit step call.
#[test]
fn render_loop_does_not_auto_run() {
    let mut sink = WasapiOutputSink::new();
    ...
    // No implicit drain — state is untouched until explicit step
}
