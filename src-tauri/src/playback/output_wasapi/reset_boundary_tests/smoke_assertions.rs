// smoke_assertions.rs
//
// Assertion helpers for the reset boundary smoke test.
...
pub fn assert_skipped_when_env_not_set(report: &WasapiResetBoundarySmokeReport) {
    ...
    assert!(
        !report.reset_attempted,
        "should not attempt reset when env not set"
    );
}

/// Assert that the report shows a skip when probe was attempted but skipped.
pub fn assert_skipped_when_attempted_but_skipped(report: &WasapiResetBoundarySmokeReport) {
    assert!(
    ...
    assert!(
        !report.reset_attempted,
        "should not attempt reset when skipped"
    );
}

/// Assert success prerequisites: endpoint through buffer_size.
pub fn assert_success_prereq_steps(report: &WasapiResetBoundarySmokeReport) {
...
pub fn assert_success_buffer_and_prefill(report: &WasapiResetBoundarySmokeReport) {
...
/// Assert start/stop/reset and single padding.
pub fn assert_success_start_stop_reset(report: &WasapiResetBoundarySmokeReport) {
    ...
    assert!(report.reset_hresult.is_some(), "reset hresult present");
    assert_eq!(report.query_mode, "reset");
    assert_eq!(report.wait_duration_ms, Some(0));
}

/// Assert that all prohibited operations remain false.
pub fn assert_prohibited_always_false(report: &WasapiResetBoundarySmokeReport) {
    assert!(!report.is_format_supported_called, "no IsFormatSupported");
    assert!(!report.loop_executed, "no silent loop");
