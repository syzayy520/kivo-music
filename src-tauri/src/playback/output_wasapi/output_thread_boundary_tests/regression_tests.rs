// output_thread_boundary_tests/regression_tests.rs
//
// Regression tests ensuring existing WASAPI boundaries are not broken
// by the output_thread_boundary module.
...
#[test]
fn wasapi_output_sink_scaffold_succeeds_after_output_thread_boundary() {
...
#[test]
fn native_output_still_uses_null_sink_after_output_thread_boundary() {
...
#[test]
fn capabilities_remain_default_after_output_thread_boundary() {
...
#[test]
fn public_native_engine_keeps_native_control_contract_after_output_thread_boundary() {
...
#[test]
fn reset_boundary_unchanged() {
    // Verify reset_boundary module still exists and has the expected function
    let report = crate::playback::output_wasapi::reset_boundary::probe_reset_boundary();
    // Without env set, it should return a skipped report
    assert!(report.skipped, "reset_boundary should skip without env");
}

#[test]
fn output_thread_boundary_stub_returns_skipped_on_non_windows() {
    // On any platform (including Windows without env), probe_output_thread_boundary
    // should return a report. On non-Windows it's always skipped.
    #[cfg(not(windows))]
    {
        let report =
            crate::playback::output_wasapi::output_thread_boundary::probe_output_thread_boundary();
