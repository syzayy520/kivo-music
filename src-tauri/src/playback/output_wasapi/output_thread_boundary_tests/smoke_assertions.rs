// smoke_assertions.rs
//
// Assertion helpers for the output thread boundary smoke test.
...
pub fn assert_skipped_when_attempted_but_skipped(report: &WasapiOutputThreadSmokeReport) {
    assert!(
...
pub fn assert_success_prereq_steps(report: &WasapiOutputThreadSmokeReport) {
...
pub fn assert_success_buffer_and_prefill(report: &WasapiOutputThreadSmokeReport) {
...
pub fn assert_success_start_stop_reset(report: &WasapiOutputThreadSmokeReport) {
...
pub fn assert_prohibited_always_false(report: &WasapiOutputThreadSmokeReport) {
    assert!(!report.output_sink_connected, "no OutputSink");
    assert!(!report.capability_exposed, "no PlaybackCapabilities");
    assert!(!report.thread_callback_registered, "no thread callback");
