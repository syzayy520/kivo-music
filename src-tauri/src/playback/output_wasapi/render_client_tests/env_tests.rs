// render_client_tests/env_tests.rs
//
// Environment variable and skip behavior tests for the WASAPI render client smoke boundary.
...
#[test]
fn render_client_smoke_without_env_skips_without_attempting() {
...
#[test]
fn render_client_smoke_with_non_one_env_skips_without_attempting() {
    // Test various non-"1" values
    for value in &["0", "true", "false", "yes", "no", "on", "off", ""] {
        env::set_var(WASAPI_RENDER_CLIENT_SMOKE_ENV, value);
...
#[test]
fn render_client_smoke_report_uses_render_client_env_name() {
    env::remove_var(WASAPI_RENDER_CLIENT_SMOKE_ENV);

    let report = probe_render_client();

    assert_eq!(
        report.opt_in_env, WASAPI_RENDER_CLIENT_SMOKE_ENV,
        "report should reference the correct env var name"
    );
    assert_eq!(
        report.opt_in_env, "KIVO_WASAPI_RENDER_CLIENT_SMOKE",
        "env var name should be KIVO_WASAPI_RENDER_CLIENT_SMOKE"
    );
}

#[cfg(not(windows))]
#[test]
fn render_client_non_windows_stub_is_safe() {
