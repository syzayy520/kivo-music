...
fn start_stop_smoke_windows_ignored_opt_in() {
    ...
    if report.skipped {
        ...
        eprintln!("start_stop smoke skipped: {:?} - {:?}", report.skipped_reason, report.error_message);
        return;
    }

    // Probe succeeded
    assert!(report.endpoint_available, "endpoint should be available on success");
    assert!(report.client_activated, "client should be activated on success");
    assert!(report.mix_format_available, "mix format should be available on success");
    assert!(report.initialize_attempted, "initialize should have been attempted on success");
    assert!(report.initialized_audio_client, "initialize should have succeeded on success");
    assert!(report.get_service_attempted, "get_service should have been attempted on success");
    assert!(report.render_client_obtained, "render_client should have been obtained on success");
    assert!(report.get_buffer_size_attempted, "get_buffer_size should have been attempted on success");
    assert!(report.buffer_size_frames.is_some(), "buffer_size_frames should be Some on success");
    assert!(report.buffer_size_frames.unwrap() >= 1, "buffer_size_frames should be >= 1 on success");
    assert!(report.get_buffer_attempted, "get_buffer should have been attempted on success");
    assert!(report.buffer_obtained, "buffer should have been obtained on success");
    assert!(report.release_buffer_attempted, "release_buffer should have been attempted on success");
    assert!(report.buffer_released, "buffer should have been released on success");
    assert!(report.used_silent_flag, "should have used silent flag on success");

    // Key assertions: requested_frames = 1
    assert_eq!(report.requested_frames, Some(1), "requested_frames should be Some(1)");
    assert_eq!(report.released_frames, Some(1), "released_frames should be Some(1)");

    // Start/Stop assertions
    assert!(report.start_attempted, "start should have been attempted on success");
    assert!(report.started_audio_client, "start should have succeeded on success");
    assert!(report.stop_attempted, "stop should have been attempted on success");
    assert!(report.stopped_audio_client, "stop should have succeeded on success");
