// reset_boundary/reset_boundary_steps.rs
//
// Step helper functions for reset boundary smoke probe.
//
// This file extracts individual WASAPI probe steps from probe_reset_boundary()
// to keep reset_boundary_windows.rs within the 220-line file limit.
//
...
) -> Result<IMMDeviceEnumerator, WasapiResetBoundarySmokeReport> {
    unsafe { CoCreateInstance(&MMDeviceEnumerator, None, CLSCTX_ALL) }.map_err(|e| {
        WasapiResetBoundarySmokeReport::skipped_with_error(
...
) -> Result<windows::Win32::Media::Audio::IMMDevice, WasapiResetBoundarySmokeReport> {
...
) -> Result<IAudioClient, WasapiResetBoundarySmokeReport> {
    unsafe { endpoint.Activate(CLSCTX_ALL, None) }.map_err(|e| {
        WasapiResetBoundarySmokeReport::endpoint_available_but_activate_failed(format!(
...
) -> Result<MixFormatGuard, WasapiResetBoundarySmokeReport> {
    let format_ptr = unsafe { audio_client.GetMixFormat() }.map_err(|e| {
        WasapiResetBoundarySmokeReport::client_activated_but_mix_format_failed(format!(
...
) -> Result<BufferGuard, WasapiResetBoundarySmokeReport> {
    let hr = unsafe { render_client.GetBuffer(frames) };
    if hr.is_ok() {
        Ok(BufferGuard::new(render_client.clone(), frames))
    } else {
        Err(WasapiResetBoundarySmokeReport::prefill_get_buffer_failed(
            fields,
            buffer_size_frames,
...
pub(super) fn release_buffer_silent(
    ...
    buffer_size_frames: u32,
    fields: super::format_fields::FormatFields,
) -> Result<(), WasapiResetBoundarySmokeReport> {
    buffer_guard.release_silent().map_err(|e| {
        WasapiResetBoundarySmokeReport::prefill_release_buffer_failed(fields, buffer_size_frames, e)
    })
}
