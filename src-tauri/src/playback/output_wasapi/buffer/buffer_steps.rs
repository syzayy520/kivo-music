// buffer_steps.rs
//
// Step helper functions for IAudioRenderClient::GetBuffer + ReleaseBuffer silent smoke.
//
// This file extracts individual WASAPI probe steps from probe_buffer()
// to keep buffer_windows.rs within the 220-line file limit.
//
...
pub(super) fn create_device_enumerator() -> Result<IMMDeviceEnumerator, WasapiBufferSmokeReport> {
    // Windows COM FFI boundary
    unsafe { CoCreateInstance(&MMDeviceEnumerator, None, CLSCTX_ALL) }.map_err(|e| {
        WasapiBufferSmokeReport::skipped_with_error(
...
) -> Result<windows::Win32::Media::Audio::IMMDevice, WasapiBufferSmokeReport> {
    // Windows COM FFI boundary
...
) -> Result<IAudioClient, WasapiBufferSmokeReport> {
    // Windows COM FFI boundary
    unsafe { endpoint.Activate(CLSCTX_ALL, None) }.map_err(|e| {
        WasapiBufferSmokeReport::endpoint_available_but_activate_failed(format!(
...
) -> Result<MixFormatGuard, WasapiBufferSmokeReport> {
    // Windows COM FFI boundary
    let format_ptr = unsafe { audio_client.GetMixFormat() }.map_err(|e| {
        WasapiBufferSmokeReport::client_activated_but_mix_format_failed(format!(
...
///
/// Returns a BufferGuard that will release the buffer on drop.
#[allow(clippy::result_large_err)]
pub(super) fn get_buffer(
...
) -> Result<BufferGuard, WasapiBufferSmokeReport> {
    // Windows COM FFI boundary
    ...
    if hr.is_ok() {
        ...
        Err(WasapiBufferSmokeReport::get_buffer_failed(
            fields,
            buffer_size_frames,
            frames,
...
pub(super) fn release_buffer_silent(
    ...
    fields: super::format_fields::FormatFields,
) -> Result<(), WasapiBufferSmokeReport> {
    // Windows COM FFI boundary
    // release buffer with silent flag
    // no audio produced
    buffer_guard.release_silent().map_err(|e| {
        WasapiBufferSmokeReport::release_buffer_failed(
            fields,
            buffer_size_frames,
            requested_frames,
            e,
