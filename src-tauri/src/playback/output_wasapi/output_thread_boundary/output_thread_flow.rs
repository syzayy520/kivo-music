// output_thread_boundary/output_thread_flow.rs
//
// WASAPI lifecycle flow for output thread boundary smoke.
//
// This file contains the internal probe that runs inside the output thread:
...
/// Run the full WASAPI lifecycle probe inside the output thread.
///
/// Returns a `WasapiOutputThreadSmokeReport` describing the outcome.
pub fn run_output_thread_flow() -> WasapiOutputThreadSmokeReport {
    ...
    if let Err(e) = stop_guard.stop() {
        return WasapiOutputThreadSmokeReport::stop_failed(
            fields,
            buffer_size_frames,
            padding_frames,
            e,
