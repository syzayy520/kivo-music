// buffer_tests/regression_tests.rs
//
// Regression tests verifying that buffer smoke does not change
...
#[test]
fn native_output_still_uses_null_sink_after_buffer_boundary() {
...
#[test]
fn capabilities_remain_default_after_buffer_boundary() {
...
fn public_native_engine_keeps_native_control_contract_after_buffer_boundary() {
    ...
    assert!(matches!(engine.seek(0), Err(PlaybackError::NoTrack(_))));
...
#[test]
fn initialize_smoke_boundary_still_does_not_get_buffer() {
    // Verify that the initialize smoke still does NOT call GetBuffer
    // This is a regression test to ensure buffer boundary is independent
    ...
    // Initialize smoke must never call GetBuffer
    assert!(
...
#[test]
fn render_client_smoke_boundary_still_does_not_get_buffer() {
    // Verify that the render_client smoke still does NOT call GetBuffer
    // This is a regression test to ensure buffer boundary is independent
    use crate::playback::output_wasapi::render_client::WASAPI_RENDER_CLIENT_SMOKE_ENV;
    use std::env;
    ...
    // Render client smoke must never call GetBuffer
    assert!(
        !report.get_buffer_called,
        "render_client smoke should never call GetBuffer"
    );
    assert!(
        !report.release_buffer_called,
        "render_client smoke should never call ReleaseBuffer"
    );
    assert!(
        !report.started_audio_client,
        "render_client smoke should never call Start"
    );
    assert!(
        !report.stopped_audio_client,
        "render_client smoke should never call Stop"
    );
    assert!(
        !report.reset_audio_client,
        "render_client smoke should never call Reset"
    );
}
