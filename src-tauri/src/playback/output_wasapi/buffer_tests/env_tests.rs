// buffer_tests/env_tests.rs
//
// Environment variable and skip behavior tests for the WASAPI buffer smoke boundary.
...
#[test]
fn buffer_smoke_without_env_skips_without_attempting() {
...
#[test]
fn buffer_smoke_with_non_one_env_skips_without_attempting() {
    // Test various non-"1" values
    for value in &["0", "true", "false", "yes", "no", "on", "off", ""] {
        env::set_var(WASAPI_BUFFER_SMOKE_ENV, value);
        ...
        assert!(
            !report.get_service_attempted,
            "get_service_attempted should be false for env value '{value}'"
        );
        assert!(
            !report.render_client_obtained,
            "render_client_obtained should be false for env value '{value}'"
        );
        assert!(
            !report.get_buffer_size_attempted,
            "get_buffer_size_attempted should be false for env value '{value}'"
        );
        assert!(
            !report.get_buffer_attempted,
            "get_buffer_attempted should be false for env value '{value}'"
        );
        assert!(
            !report.buffer_obtained,
            "buffer_obtained should be false for env value '{value}'"
        );
        assert!(
            !report.release_buffer_attempted,
            "release_buffer_attempted should be false for env value '{value}'"
        );
        assert!(
            !report.buffer_released,
            "buffer_released should be false for env value '{value}'"
        );
    }
...
#[test]
fn buffer_smoke_report_uses_buffer_env_name() {
    env::remove_var(WASAPI_BUFFER_SMOKE_ENV);

    let report = probe_buffer();

    assert_eq!(
        report.opt_in_env, WASAPI_BUFFER_SMOKE_ENV,
        "report should reference the correct env var name"
    );
    assert_eq!(
        report.opt_in_env, "KIVO_WASAPI_BUFFER_SMOKE",
        "env var name should be KIVO_WASAPI_BUFFER_SMOKE"
    );
}

#[cfg(not(windows))]
#[test]
fn buffer_non_windows_stub_is_safe() {
    ...
    assert!(!report.get_buffer_attempted);
    assert!(!report.buffer_obtained);
    assert!(!report.release_buffer_attempted);
    assert!(!report.buffer_released);
