...
#[test]
fn resume_without_track_is_idle_noop_success() {
    let mut engine = KivoNativeEngine::new();

    let state = engine
        .resume()
        .expect("resume without track should succeed");
...
#[test]
fn resume_loaded_idle_is_idle_noop_success() {
    let mut engine = KivoNativeEngine::new();
    let (track, path) = wav_track();

    engine.load(track).expect("load should succeed");
    let before = engine.current_state();
    let state = engine.resume().expect("resume loaded idle should succeed");
...
fn resume_from_paused_transitions_to_playing() {
    ...
    let state = engine.resume().expect("resume from paused should succeed");
    let current = engine.current_state();

    assert!(matches!(state.status, PlaybackStatus::Playing));
    assert!(matches!(current.status, PlaybackStatus::Playing));
    assert!(state.error.is_none());
...
#[test]
fn resume_when_already_playing_is_idempotent() {
    let mut engine = KivoNativeEngine::new();
    let (track, path) = multi_frame_wav_track();
    ...
    let state = engine
        .resume()
        .expect("resume while playing should succeed");
...
#[test]
fn resume_when_stopped_is_stopped_noop_success() {
    let mut engine = KivoNativeEngine::new();
    let (track, path) = wav_track();
    ...
    let state = engine.resume().expect("resume stopped should succeed");
...
#[test]
fn resume_preserves_current_track_volume_timeline_metadata() {
    let mut engine = KivoNativeEngine::new();
    let (track, path) = multi_frame_wav_track();
    ...
    let state = engine.resume().expect("resume should succeed");
...
#[test]
fn resume_does_not_change_seek_contract() {
    let mut engine = KivoNativeEngine::new();
...
#[test]
fn resume_noop_states_preserve_error_policy_where_observable() {
    let mut engine = KivoNativeEngine::new();

    let _ = engine.play();
    let before = engine.current_state();
    let state = engine.resume().expect("resume noop should succeed");
