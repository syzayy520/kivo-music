...
pub(super) fn write_test_wav() -> String {
    let path = std::env::temp_dir().join(format!(
        "kivo-native-pipeline-decode-step-{}-{}.wav",
        std::process::id(),
        unique_suffix()
    ));
...
pub(super) fn write_long_test_wav() -> String {
    let path = std::env::temp_dir().join(format!(
        "kivo-native-pipeline-long-decode-step-{}-{}.wav",
        std::process::id(),
        unique_suffix()
    ));
    let spec = hound::WavSpec {
