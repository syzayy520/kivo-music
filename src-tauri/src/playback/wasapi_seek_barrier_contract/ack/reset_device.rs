...
/// Ack for device reset completion.
///
/// Binds the observed generation.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ResetDeviceAck {
...
impl ResetDeviceAck {
    ...
    /// Whether this ack satisfies the device reset barrier for the expected generation.
    ///
    /// Satisfied only when status is Completed AND generation matches.
    pub fn satisfies_device_reset_barrier(&self, expected: SeekOutputGeneration) -> bool {
        self.status == WasapiSeekBarrierAckStatus::Completed
