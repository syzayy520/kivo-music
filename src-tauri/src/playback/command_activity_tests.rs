...
#[test]
fn run_state_command_records_error_on_failure() {
    let manager = PlaybackManagerState::default();
    let activity = PlaybackActivityLogState::default();
...
#[test]
fn run_track_command_records_track_changed_on_success() {
    let manager = PlaybackManagerState::default();
    let activity = PlaybackActivityLogState::default();

    let result = run_track_command(&manager, &activity, |_| Ok(PlaybackState::default()));
