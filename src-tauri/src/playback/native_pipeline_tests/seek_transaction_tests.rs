...
fn seek_transaction_success_clears_buffer() {
    let (mut pipeline, path) = open_pipeline_with_decoded_frame();
    assert_eq!(pipeline.buffer.len(), 1, "buffer should have 1 frame after decode");

    pipeline.seek_decoder(2).expect("seek should succeed");

    assert_eq!(pipeline.buffer.len(), 0, "buffer should be empty after successful seek");
...
fn seek_transaction_success_updates_clock() {
    ...
    assert_eq!(pipeline.clock.position_ms(), 2, "clock should reflect seek position");
...
fn seek_transaction_decoder_not_open_no_mutation() {
    ...
    assert_eq!(
        state_after.decoder_state.phase,
        state_before.decoder_state.phase,
...
fn seek_transaction_decoder_seek_failure_restores_decoder_state() {
    ...
    assert_eq!(session_after.track_id, session_track_id, "session track_id should be restored");
    assert_eq!(session_after.last_position_ms, session_position, "session position should be restored");
    assert_eq!(session_after.decoded_frame_count, session_frame_count, "session frame count should be restored");
    assert_eq!(state_after.decoder_state.phase, decoder_phase_before, "decoder_state phase should be restored");
    assert_eq!(state_after.decoder_state.last_error, decoder_error_before, "decoder_state last_error should be restored");
...
fn seek_transaction_failure_does_not_clear_buffer() {
    ...
    assert!(buffer_len_before > 0, "buffer should have frames before seek");
